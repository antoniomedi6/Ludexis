    ...
    <x-modal wire:model="showModal" maxWidth="4xl">
        <div class="space-y-4 sm:space-y-8">
            ...
            <header class="text-center">
                <div class="mb-2 sm:mb-5 inline-flex h-10 w-10 sm:h-16 sm:w-16 items-center justify-center rounded-full bg-cyan-50 text-brand shadow-inner dark:bg-darkbox-main"
                    aria-hidden="true">
                    <i class="fa-solid fa-dice-d20 text-xl sm:text-3xl"></i>
                </div>
                <h3 class="text-xl sm:text-3xl font-black uppercase leading-none tracking-tight text-gray-900 dark:text-white">
                    Ruleta de juegos
                </h3>
                <p class="mx-auto mt-2 sm:mt-3 max-w-2xl text-xs sm:text-sm font-medium leading-5 sm:leading-6 text-gray-500 dark:text-gray-400 hidden sm:block">
            ...
            <div class="grid gap-5 sm:gap-6 lg:grid-cols-5">
                {{-- CONTROLES --}}
                <form wire:submit.prevent="pickGame" class="space-y-4 sm:space-y-6 lg:col-span-2"
                    aria-label="Filtros del selector aleatorio">
                    <fieldset class="flex flex-col gap-2">
                        <legend
                            class="text-xs font-black uppercase tracking-widest mb-2 sm:mb-4 text-gray-500 dark:text-gray-400">
                            Origen
                        </legend>
                        ...
                        <label class="cursor-pointer relative group">
                            ...
                            <div
                                class="flex items-center gap-2 sm:gap-3 rounded-2xl border border-gray-200 bg-gray-50 p-2.5 sm:p-3 text-gray-400 transition-all duration-300 peer-checked:border-cyan-500 peer-checked:bg-cyan-50 peer-checked:text-cyan-700 hover:border-cyan-300 dark:border-gray-800 dark:bg-darkbox-main dark:text-gray-500 dark:peer-checked:border-cyan-600 dark:peer-checked:bg-cyan-900/20 dark:peer-checked:text-cyan-400 dark:hover:border-cyan-700">
                                <span
                                    class="flex size-8 sm:size-10 shrink-0 items-center justify-center rounded-xl bg-white text-current shadow-sm transition-colors duration-300 dark:bg-darkbox-card"
                                    aria-hidden="true">
                                    <i class="fa-solid fa-globe"></i>
                                </span>
                                <span class="min-w-0">
                                    <span class="block text-xs sm:text-sm font-black text-gray-900 dark:text-white">
                                        Catálogo completo
                                    </span>
                                    <span class="hidden sm:block text-xs font-bold text-gray-500 dark:text-gray-400">
                    ...
                    <div class="space-y-2" x-show="source === 'list'" x-transition x-cloak>
                        ...
                        <select id="random-list" x-model="listId"
                            class="w-full cursor-pointer rounded-xl border border-gray-200 bg-white px-3 sm:px-4 py-2.5 sm:py-3 text-sm font-bold text-gray-900 shadow-sm transition-colors focus:border-cyan-500 focus:outline-none focus:ring-2 focus:ring-cyan-500 dark:border-darkbox-border dark:bg-darkbox-main dark:text-white">
                    ...
                    <button type="submit" wire:loading.attr="disabled" wire:target="pickGame"
                        class="flex w-full items-center justify-center gap-3 rounded-xl bg-cyan-700 py-3 sm:py-3.5 text-xs font-black uppercase tracking-widest text-white shadow-lg transition-all active:scale-95 hover:bg-cyan-600 focus:outline-none focus:ring-2 focus:ring-cyan-500 disabled:cursor-not-allowed disabled:opacity-50 dark:bg-cyan-600 dark:hover:bg-cyan-500"
                ...
                <section class="lg:col-span-3" aria-live="polite">
                    @if ($randomGame)
                        <article
                            ...
                            <div class="grid gap-0 md:grid-cols-5">
                                <div
                                    class="relative h-48 sm:h-72 overflow-hidden bg-gray-100 dark:bg-darkbox-main md:col-span-2">
                                ...
                                <div class="flex flex-col justify-center p-4 sm:p-5 md:col-span-3 md:p-6">
                                    <span class="mb-2 text-[10px] font-black uppercase tracking-widest text-brand">
                                        Recomendado
                                    </span>
                                    <h4
                                        class="text-lg sm:text-2xl font-black leading-tight text-gray-900 dark:text-white break-words">
                                        {{ $randomGame->title }}
                                    </h4>
                                    ...
                                    <div class="mt-5 flex flex-col sm:flex-row gap-2 w-full">
                                        <button type="button" wire:click="pickGame"
                                            class="flex-1 bg-gray-200 dark:bg-darkbox-main hover:bg-gray-300 dark:hover:bg-darkbox-card text-gray-800 dark:text-gray-300 font-black py-3 rounded-xl uppercase text-xs tracking-widest transition-colors focus:outline-none focus:ring-2 focus:ring-gray-500 flex items-center justify-center gap-2">
                                            <i class="fa-solid fa-rotate" aria-hidden="true"></i>
                                            Repetir
                                        </button>
                                        <a href="{{ route('games.show', $randomGame->slug) }}" wire:navigate
                                            class="flex-1 bg-cyan-700 hover:bg-cyan-600 dark:bg-cyan-600 dark:hover:bg-cyan-500 text-white font-black py-3 rounded-xl uppercase text-xs tracking-widest shadow-lg transition-all active:scale-95 focus:outline-none focus:ring-2 focus:ring-cyan-500 flex items-center justify-center gap-2">
                                            Ficha
                    ...
                    @else
                        <div
                            class="flex min-h-56 sm:min-h-96 flex-col items-center justify-center rounded-3xl border-2 border-dashed border-gray-200 bg-gray-50 p-4 sm:p-8 text-center transition-colors duration-300 dark:border-darkbox-border dark:bg-darkbox-main">
                            <div class="mb-3 sm:mb-5 flex h-12 w-12 sm:h-20 sm:w-20 items-center justify-center rounded-full bg-white text-brand shadow-sm dark:bg-darkbox-card"
                                aria-hidden="true">
                                <i class="fa-solid fa-gamepad text-xl sm:text-3xl"></i>
                            </div>
                            <h4 class="text-lg sm:text-2xl font-black text-gray-900 dark:text-white">
                                Todavía no hay tirada
                            </h4>
                            <p class="mt-2 sm:mt-3 max-w-md text-xs sm:text-sm font-medium leading-5 sm:leading-6 text-gray-500 dark:text-gray-400">
