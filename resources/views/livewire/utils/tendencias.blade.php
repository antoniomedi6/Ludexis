<section aria-labelledby="tendencias-heading">
    ...
    {{-- GRID DE TENDENCIAS --}}
    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4" role="list">

        @foreach ($popularGames as $item)
            <div role="listitem">
                <a href="{{ route('games.show', $item->slug) }}"
                    class="block w-full relative aspect-[3/4] rounded-xl overflow-hidden group border border-gray-200 dark:border-gray-800 hover:border-cyan-500 dark:hover:border-cyan-500 transition shadow-md dark:shadow-lg focus:outline-none focus:ring-4 focus:ring-cyan-500">
                    <img src="{{ $item->cover_url }}" alt="Portada del juego {{ $item->title }}" loading="lazy"
                        class="w-full h-full object-cover group-hover:scale-105 transition duration-500" />

                    <div class="absolute top-2 right-2">
                        <span
                            class="bg-gray-900/90 backdrop-blur text-cyan-400 font-black text-xs md:text-sm px-2 py-0.5 rounded-lg border border-gray-700 shadow-lg drop-shadow-md"
                    ...
                    <div class="absolute inset-0 bg-gradient-to-t from-gray-950 via-gray-950/40 to-transparent flex flex-col justify-end p-3 pointer-events-none"
                        aria-hidden="true">
                        <h3
                            class="font-bold text-[10px] sm:text-xs leading-tight text-white drop-shadow-md line-clamp-2">
                            {{ $item->title }}
                        </h3>
                    </div>
                </a>
