            ...
            <div x-show="show" x-transition.opacity.duration.200ms style="display: none;" id="search-users-results"
                ...
                @foreach ($users as $item)
                    <div role="option" wire:key="search-user-row-{{ $item->id }}"
                        class="flex items-center justify-between gap-2 p-3 border-b border-gray-100 dark:border-gray-700/50 last:border-0 hover:bg-gray-50 dark:hover:bg-gray-700 focus-within:bg-gray-50 dark:focus-within:bg-gray-700 transition-colors group">
                        <a href="{{ route('profile', $item->id) }}" wire:navigate
                            ...
                            <img src="{{ $item->profile_photo_url }}" alt="Avatar de {{ $item->name }}"
                                ...
                                loading="lazy">
                            <h4 class="min-w-0 flex-1 truncate text-sm font-black text-gray-900 dark:text-white">
                                {{ $item->name }}
                            </h4>
                        </a>
