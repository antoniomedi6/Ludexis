<div class="flex items-center gap-2" x-data @click.stop>
    ...
    @auth
        <button type="button" wire:click="toggleSpoiler"
            ...
            aria-label="{{ $image->is_spoiler ? 'Quitar spoiler' : 'Marcar como spoiler' }}"
            aria-pressed="{{ $image->is_spoiler ? 'true' : 'false' }}">
            <i class="fa-solid {{ $image->is_spoiler ? 'fa-eye' : 'fa-eye-slash' }} text-xs" aria-hidden="true"></i>
        </button>
