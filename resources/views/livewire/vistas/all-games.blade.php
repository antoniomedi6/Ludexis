    ...
    <div class="flex-1 flex overflow-hidden">
        <aside class="w-72 bg-[#151821] border-r border-gray-800 shrink-0 overflow-y-auto">
            <div class="p-6">
                ...
                {{-- Plataformas con buscador local en Alpine --}}
                <div class="mb-8" x-data="{ search: '' }">
                    ...
                    <div class="space-y-2 max-h-40 overflow-y-auto pr-2 custom-scrollbar">
                        @foreach ($allPlatforms as $platform)
                            {{-- Alpine oculta los elementos que no coinciden con la búsqueda --}}
                            <label
                ...
                {{-- Géneros Top 5 + Expandir con Alpine --}}
                <div class="mb-8" x-data="{ expanded: false }">
                    ...
                    <div class="flex flex-wrap gap-2">
                        {{-- Top Géneros (Visibles siempre) --}}
                        @foreach ($topGenres as $genre)
                            <button
                                class="bg-[#1a1d27] text-gray-400 border border-gray-800 hover:border-gray-600 px-3 py-1.5 rounded-lg text-xs font-bold transition">
                                {{ $genre->name }}
                            </button>
                        @endforeach

                        {{-- Resto de Géneros ocultos --}}
        ...
        <main class="flex-1 overflow-y-auto p-6 md:p-8 bg-[#0f1117]">
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
                ...
                <select wire:model.live="orderBy"
                    class="bg-[#1a1d27] border border-gray-800 text-white text-sm rounded-xl px-4 py-2.5 font-bold focus:outline-none focus:border-cyan-500/50 appearance-none cursor-pointer">
